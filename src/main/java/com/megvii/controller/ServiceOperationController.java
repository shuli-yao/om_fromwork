    ...
    public String allXDBPhoto(Integer begin, Integer end) {
        ...
        Thread thread = new Thread(new Runnable() {
            ...
            public void run() {
                ...
                Integer queryMaxNumber = systemConfig.getQueryMaxSize(); //每页最大查询数量
                Integer beginNumber = 0;    //开始行数
                Integer topBeginNumber = 0;   //上次开始行数
                Integer endNumber = beginNumber + queryMaxNumber;   //结束行数
                Integer performNumber = 0;    //执行总数
                ...
                while (true) {
                    ...
                    log.info("落地完毕:" + it + "落地消耗时间：" + (System.currentTimeMillis() -beginTime) + "ms，当前已执行总数:" + performNumber);
